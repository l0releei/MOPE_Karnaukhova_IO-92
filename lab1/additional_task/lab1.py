...
result = max(list_y)
culc = round(int(np.mean(list_y)))

def nearest(lst, target):
  return min(lst, key=lambda x: abs(x-target))

culc2 = nearest(list_y, culc)
num = list_y.index(culc2)
result2 = list_y[num+1]
...
print(table)
print("Середнє Y: {0}".format(culc))
print("Найближче значення Y у списку: {0}".format(culc2))
print("→ Y` ,де Y` - середнє Y: {0}".format(result2))
print("Y max: {0}".format(result))
